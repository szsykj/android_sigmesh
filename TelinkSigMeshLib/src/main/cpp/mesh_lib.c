...
extern void mesh_tx_reliable_stop_report(u16 op, u32 rsp_max, u32 rsp_cnt) {
...
Java_com_telink_sig_mesh_lib_MeshLib_isOpReliable(JNIEnv *env, jobject j,
                                                  jint opcode) {
    LOGD("isOpReliable");
    return (jboolean) (get_op_rsp((u16) opcode) != STATUS_NONE);
}
...
// for fast provision todo
// 实现下这两个接口，用来存储当前使用的unicast adr，上面那个是获取，下面那个是设置
